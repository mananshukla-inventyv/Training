use crate::{data::USER_DATA, helper_fun::{create_user_struct, ser::{user_service_server::UserService, Empty, Id, Response as resp, UserData}, string_to_language, string_to_status}};
...
impl UserService for UServices{
    async fn get_user(&self,req:Request<Id>)->Result<Response<resp>,Status>{
        ...
        if let Some(user_to_be_displayed)= USER_DATA.write().unwrap().get_mut(&id.to_string()).cloned(){
    ...
    async fn update_user(&self,req:Request<UserData>)->Result<Response<resp>,Status>{
        ...
        if let Some(item)=USER_DATA.write().unwrap().get_mut(&user_to_be_updated.id.to_string()){
    ...
    async fn create_user(&self,req:Request<UserData>)->Result<Response<resp>,Status>{
        ...
        if !USER_DATA.read().unwrap().contains_key(&user_to_be_added.id.to_string()){
            USER_DATA.write().unwrap().insert(user_to_be_added.id.to_string().clone(), user_to_be_added.clone());
            Ok(Response::new(resp{
                status:1000,
