use crate::helper_fun::{create_student_struct, ser::{student_service_server::StudentService, Empty, Id, Response as resp, StudentData as stdd}};
...
impl StudentService for StdService{
    async fn get_student(&self,req:Request<Id>)->Result<Response<resp>,Status>{
        ...
        if let Some(student_to_be_displayed)= STUDENT_DATA.write().unwrap().get_mut(&id.to_string()).cloned(){
    ...
    async fn update_student(&self,req:Request<stdd>)->Result<Response<resp>,Status>{
        ...
        if let Some(item)=STUDENT_DATA.write().unwrap().get_mut(&student_to_be_updated.id.to_string()){
    ...
    async fn create_student(&self,req:Request<stdd>)->Result<Response<resp>,Status>{
        ...
        if !STUDENT_DATA.read().unwrap().contains_key(&std_to_be_added.id.to_string()){
            STUDENT_DATA.write().unwrap().insert(std_to_be_added.id.to_string().clone(), std_to_be_added.clone());
            Ok(Response::new(resp{
                status:1000,
