use crate::helper_fun::{
    create_employee_struct,
    ser::{
        employee_service_server::EmployeeService, EmployeeData as emp, Empty, Id, Response as resp,
    },
};
...
impl EmployeeService for EmpService {
    async fn get_employee(&self, req: Request<Id>) -> Result<Response<resp>, Status> {
        ...
        if let Some(employee_to_be_displayed) = EMPLOYEE_DATA
            .write()
            .unwrap()
    ...
    async fn update_employee(&self, req: Request<emp>) -> Result<Response<resp>, Status> {
        ...
        if let Some(item) = EMPLOYEE_DATA
            .write()
            .unwrap()
            .get_mut(&employee_to_be_updated.id.to_string())
        {
    ...
    async fn create_employee(&self, req: Request<emp>) -> Result<Response<resp>, Status> {
        ...
        if !EMPLOYEE_DATA
            .read()
            .unwrap()
            .contains_key(&emp_to_be_added.id.to_string())
        {
            EMPLOYEE_DATA.write().unwrap().insert(
                emp_to_be_added.id.to_string().clone(),
                emp_to_be_added.clone(),
            );
            Ok(Response::new(resp {
                status: 1000,
